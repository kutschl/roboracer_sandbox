...
import configparser
from global_planner.gco.opt_mintime_traj.opt_mintime import opt_mintime
from global_planner.gco.helper_funcs_glob.import_track import import_track
...
from global_planner.gco.helper_funcs_glob.result_plots import result_plots
from global_planner.trajectory_planning_helpers.import_veh_dyn_info import import_veh_dyn_info
from global_planner.trajectory_planning_helpers.opt_min_curv import opt_min_curv
from global_planner.trajectory_planning_helpers.iqp_handler import iqp_handler
from global_planner.trajectory_planning_helpers.opt_shortest_path import opt_shortest_path
from global_planner.trajectory_planning_helpers.create_raceline import create_raceline
from global_planner.trajectory_planning_helpers.calc_normal_vectors_ahead import calc_normal_vectors_ahead
...
from global_planner.trajectory_planning_helpers.progressbar import progressbar
from global_planner.trajectory_planning_helpers.calc_head_curv_an import calc_head_curv_an, calc_derivatives_an
from global_planner.trajectory_planning_helpers.calc_splines import calc_splines
...
def trajectory_optimizer(input_path: str,
                         ...
                         plot: bool = False,
                         recompute_velocity_profile: bool=False,
                         reopt_mintime: bool=False,
    ...
    mintime_opts = {"tpadata": None,
                    "warm_start": False,
                    "var_friction": None,
                    "reopt_mintime_solution": reopt_mintime,
                    "recalc_vel_profile_by_tph": recompute_velocity_profile}
    
    # lap time calculation table ---------------------------------------------------------------------------------------
    lap_time_mat_opts = {"use_lap_time_mat": True,             # calculate a lap time matrix (top speeds and scales)
                         "gg_scale_range": [0.3, 1.0],          # range of gg scales to be covered
                         "gg_scale_stepsize": 0.05,             # step size to be applied
                         "top_speed_range": [40.0, 54.0],     # range of top speeds to be simulated [in km/h]
    ...
    if not (curv_opt_type == 'mintime' and not mintime_opts["recalc_vel_profile_by_tph"]):
        ggv, ax_max_machines = import_veh_dyn_info(ggv_import_path=file_paths["ggv_file"],
    ...
    if curv_opt_type == 'mintime' and pars["optim_opts"]["safe_traj"] \
        ...
        if ggv is None:
            ggv = import_veh_dyn_info(ggv_import_path=file_paths["ggv_file"],
    ...
    if curv_opt_type == 'mincurv':
        ...
        alpha_opt, reftrack_interp, normvec_normalized_interp, _, _, _, _ = iqp_handler(reftrack=reftrack_interp,
                        normvectors=normvec_normalized_interp,
                        A=a_interp,
                        ...
                        iters_min=2,
                        curv_error_allowed=curvature_limit,
                        iters_max=iter_limit)
    ...
    if curv_opt_type == 'mintime' and mintime_opts["reopt_mintime_solution"]:
        ...
        reftrack_interp, normvec_normalized_interp, a_interp = prep_track(reftrack_imp=racetrack_mintime,
                                                                            reg_smooth_opts=pars["reg_smooth_opts"],
                                                                            stepsize_opts=pars["stepsize_opts"],
                                                                            debug=False,
        ...
        alpha_opt = opt_min_curv(reftrack=racetrack_mintime_reopt,
                                   normvectors=normvec_normalized_interp,
                                   A=a_interp,
                                   kappa_bound=pars["veh_params"]["curvlim"],
    ...
    if alpha_opt.shape[0] > reftrack_interp.shape[0]:
        alpha_opt_ = alpha_opt[:reftrack_interp.shape[0]]
    else: alpha_opt_ = alpha_opt
    ...
    first_deriv, second_deriv = calc_derivatives_an(
    ...
    if lap_time_mat_opts["use_lap_time_mat"]:
        ...
        for i, top_speed in enumerate(top_speeds):
            for j, ggv_scale in enumerate(ggv_scales):
                progressbar(i*ggv_scales.shape[0] + j,
    ...
    if plot:
        ...
        if plot_opts["imported_bounds"]:
            ...
            _, _, _, normvec_imp = calc_splines(path=np.vstack((reftrack_imp[::n_skip, 0:2],
                                                                                 reftrack_imp[0, 0:2])))
