...
class MinimumTimeWidget(QWidget):
    ...
    def __init__(self, map_path: str):
        ...
        self.global_optimizer = GlobalTrajectoryOptimizer(map_path, traj_name="min_time")
        ...
        param_name = "centerline_length"
        label = QLabel(param_name)
        line_edit = QLineEdit(str(self.global_optimizer.parameters[param_name]))

        line_edit.setReadOnly(False)  # Make it read-only for now
        line_edit.textChanged.connect(partial(self.set_parameter, name=param_name, parser=lambda x: float(x)))
        form_layout.addRow(label, line_edit)
        self.param_widgets[param_name] = line_edit

        param_name = "safety_width"
        label = QLabel(param_name)
        line_edit = QLineEdit(str(self.global_optimizer.parameters[param_name]))
        ...
        param_name = "occ_grid_threshold"
        label = QLabel(param_name)
        line_edit = QLineEdit(str(self.global_optimizer.parameters[param_name]))

        line_edit.setReadOnly(False)  # Make it read-only for now
        line_edit.textChanged.connect(partial(self.set_parameter, name=param_name, parser=lambda x: int(x)))
        form_layout.addRow(label, line_edit)
        self.param_widgets[param_name] = line_edit

        param_name = "map_editor_mode"
        label = QLabel(param_name)
        line_edit = QLineEdit(str(self.global_optimizer.parameters[param_name]))
        line_edit.setReadOnly(False)  # Make it read-only for now
        line_edit.textChanged.connect(partial(self.set_parameter, name=param_name, parser=lambda x: x.lower()=="true"))
        form_layout.addRow(label, line_edit)
        self.param_widgets[param_name] = line_edit

        param_name = "recompute_mintime"
        label = QLabel(param_name)
        line_edit = QLineEdit(str(self.global_optimizer.parameters[param_name]))
        line_edit.setReadOnly(False)  # Make it read-only for now
        line_edit.textChanged.connect(partial(self.set_parameter, name=param_name, parser=lambda x: x.lower()=="true"))
        ...
        line_edit = QLineEdit(str(self.global_optimizer.parameters[param_name]))
        line_edit.setReadOnly(False)  # Make it read-only for now
        line_edit.textChanged.connect(partial(self.set_parameter, name=param_name,))
        form_layout.addRow(label, line_edit)
        self.param_widgets[param_name] = line_edit
        ...
        self.skeleton_label.setFont(header_font)
        ...
        self.centerline_label.setFont(header_font)
        ...
        self.tb_label.setFont(header_font)
        ...
        self.glb_traj_label.setFont(header_font)
