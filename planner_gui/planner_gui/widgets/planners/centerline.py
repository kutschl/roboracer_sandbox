...
from global_planner.wrapper import CenterlineTrajectoryOptimizer
...
class CenterlineWidget(QWidget):
    ...
    def __init__(self, map_path: str):
        ...
        self.global_optimizer = CenterlineTrajectoryOptimizer(map_path)
        ...
        self._set_parameter("smoothing_iters", form_layout, lambda x: int(x))
        self._set_parameter("filter_size_ratio", form_layout, lambda x: float(x))
        self._set_parameter("filter_size_iter_scale", form_layout, lambda x: float(x))
        self._set_parameter("stepsize_prep", form_layout, lambda x: float(x))
        self._set_parameter("stepsize_reg", form_layout, lambda x: float(x))
        self._set_parameter("stepsize_interp_after_opt", form_layout, lambda x: float(x))
        self._set_parameter("k_reg", form_layout, lambda x: float(x))
        self._set_parameter("s_reg", form_layout, lambda x: float(x))
        self._set_parameter("v_max", form_layout, lambda x: float(x))
        self._set_parameter("dragcoeff", form_layout, lambda x: float(x))
        self._set_parameter("mass", form_layout, lambda x: float(x))
        self._set_parameter("length", form_layout, lambda x: float(x))
        self._set_parameter("width", form_layout, lambda x: float(x))
        self._set_parameter("dyn_model_exp", form_layout, lambda x: float(x))
        self._set_parameter("vel_profile_conv_filt_window", form_layout, lambda x: float(x))
        ...
        self._set_parameter("ggv_file", form_layout, lambda x: str(x))
        self._set_parameter("ax_max_machines_file", form_layout, lambda x: str(x))
    ...
    def generate_centerline(self):
        if not self.global_optimizer.compute_centerline():
    ...
    def generate_global_trajectory(self):
        if not self.global_optimizer.compute_raceline():
        ...
        self.global_optimizer.raceline_computed = self.global_optimizer.global_trajectory_pixel
        self.racelineComputed.emit()
        #self.global_optimizer.save_ros2_msgs()
        return True
